...
class TestImageData:
    ...
    @pytest.mark.parametrize("file, frame", [
        (EX_DATA / "images/gp3/0001.jpg", 1),
        (EX_DATA / "images/gp3/0506.jpg", 506)
    ])
    def test_open_image_folder(self, monkeypatch: MonkeyPatch, qtbot: QtBot,
                               loaded_images: img_data.ImageData,
                               file: pathlib.Path, frame: int):
        if not file.exists():
        ...
        signals = [loaded_images.data_loaded,
                   loaded_images.next_img[int, int]]
    ...
    @pytest.mark.parametrize(
        "start,direction",
        list(itertools.product([0, 6, 24], [-1, 1, -3, 3]))
    )
    def test_next_image(self, loaded_images: img_data.ImageData, qtbot: QtBot,

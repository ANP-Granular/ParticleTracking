...
def test_insert_missing_rods():
    ...
    expected_insertions = [*(11 * [float("NaN")]), *(8 * [-1.]), 0, 0]
    cols = [col for col in inserted.columns
            if col not in ["frame", "particle", "color"]]
    for num in inserted_numbers:
        for color in inserted.color.unique():
            np.testing.assert_allclose(
                inserted.loc[(inserted.particle == num) &
                             (inserted.color == color),
                             cols].values.tolist()[0],
                expected_insertions
            )
...
def test_replace_missing_rods():
    ...
    cols_2d = [col for col in changed_cols
    ...
    pd.testing.assert_frame_equal(test_data.loc[~nan_selection],
                                  replaced.loc[~nan_selection])
    pd.testing.assert_frame_equal(test_data.loc[nan_selection, unchanged_cols],
                                  replaced.loc[nan_selection, unchanged_cols])
    assert (replaced.loc[nan_selection, cols_seen] == 0.).all(None)
    assert (replaced.loc[nan_selection, cols_2d] == -1.).all(None)
...
def test_add_points(cam_id: str, frame: int):
    ...
    for k, v in test_points.items():
        ...
        assert (inserted.loc[k, frame][cam_cols].to_numpy() ==

...
class RodData(data.PositionData):
    ...
    **list** -> List of 'camera' IDs on which a rod can be
    ...
    def show_particles(self) -> bool:
        ...
        bool
        """
        return self._show_particles

    @show_particles.setter
    def show_particles(self, flag: bool) -> None:
        self._show_particles = flag
    ...
    def open_rod_folder(self, chosen_folder: Path) -> bool:
        ...
        global rod_data
        lock.lockForWrite()
        rod_data, found_colors = self.get_color_data(self.input)
        ...
        self.data_loaded[Path, Path, list].emit(
            self.input, self.output, found_colors
        )
    ...
    def save(self, temp_only: bool = False):
        ...
        if not temp_only:
            ...
            elif self.output == self.input and not self._allow_overwrite:
                msg = QMessageBox()
                msg.setWindowIcon(QtGui.QIcon(fl.icon_path()))
                msg.setIcon(QMessageBox.Warning)
                msg.setWindowTitle("Rod Tracker")
        ...
        for color in rod_data.color.unique():
            ...
            if self.logger is not None and not temp_only:
                action = lg.FileAction(out_file, lg.FileActions.SAVE)
                action.parent_id = self.logger_id
                self.logger.add_action(action)
        lock.unlock()
        if not temp_only:
            self.saved.emit()
    ...
    def set_frame(self, frame: int, _: int = None):
    ...
    def provide_data(self, data_2d: bool = True, data_3d: bool = True):
        ...
        if self._show_particles and data_2d:
            out_2d = out_data
            ...
            # FIXME: new signature does not provide the color argument
            # self.position_data_2d.emit(
            #     out_2d[self.cols_2D].copy(), self.color_2D
            # )
            self.position_data_2d.emit(out_2d[self.cols_2D].copy())
            if not len(out_2d):
                _logger.info(
    ...
    @QtCore.pyqtSlot(actions.NumberChangeActions, int, int, str)
    @QtCore.pyqtSlot(actions.NumberChangeActions, int, int, str, str, int)
    def catch_number_switch(
        self,
        mode: actions.NumberChangeActions,
        old_id: int,
        new_id: int,
        cam_id: str,
        color: str = None,
    ...
    def update_2D_data(self, class_ID: str = None, particle_ID: int = None):
        ...
        # TODO: class_ID being None shall select all available colors!
        if class_ID is None and particle_ID is None:
            _logger.info(
                "update_2D_data() Not implemented to display all available "
                "rods."
            )
            return
        self.provide_data(data_3d=False)
    ...
    def update_settings(self, key: str, new_value: Any) -> None:
        ...
        if key == "Rods.position_scaling" and POSITION_SCALING != new_value:
            settings_changed = True
            POSITION_SCALING = new_value
...
def rod_number_swap(
    ...
    if mode == actions.NumberChangeActions.ALL:
        rod_data.loc[
            (tmp_set.color == color)
    ...
    elif mode == actions.NumberChangeActions.ALL_ONE_CAM:
    ...
    elif mode == actions.NumberChangeActions.ONE_BOTH_CAMS:
        rod_data.loc[
            (tmp_set.color == color)
            & (tmp_set.particle == previous_id)
            & (tmp_set.frame == frame),

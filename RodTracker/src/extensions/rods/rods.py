...
class RodNumber(QtWidgets.QLineEdit):
    ...
    def copy(self):
        ...
        RodNumberWidget
        """
        copied = RodNumber(self.color, self.parent(), self.text(), self.pos())
    ...
    def update_settings(self, key: str, new_value) -> None:
        ...
        if "Rods.number_size" == key:
            settings_changed = True
            self._number_size = new_value
        if "Rods.number_color" == key:
            settings_changed = True
            self._number_color = new_value
        if "Rods.boundary_offset" == key:
            settings_changed = True
            self._boundary_offset = new_value
        if "Rods.rod_thickness" == key:
            settings_changed = True
            self._rod_thickness = new_value
        if "Rods.rod_color" == key:
            settings_changed = True
            self._rod_color = new_value
    ...
    @classmethod
    def update_defaults(cls, key: str, new_value) -> None:
        """Catches updates of the settings from a :class:`.Settings` class.
        ...
        """
        if "Rods.number_size" == key:
            cls._number_size = new_value
        if "Rods.number_color" == key:
            cls._number_color = new_value
        if "Rods.boundary_offset" == key:
            cls._boundary_offset = new_value
        if "Rods.rod_thickness" == key:
            cls._rod_thickness = new_value
        if "Rods.rod_color" == key:
            cls._rod_color = new_value

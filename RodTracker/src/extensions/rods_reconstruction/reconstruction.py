...
from .reconstruction_ui import Ui_reconstruction_ui
...
class Plotter(QtCore.QRunnable):
    ...
    def plot_reprojection_errors(
        ...
        if data is None:
            _logger.error(
                f"Insufficient position data was provided: " f"{data}"
            )
        if calibration is None:
            _logger.error(
                f"Insufficient calibration data was provided:"
                f" {calibration}"
            )
        if position_scaling is None:
            position_scaling = 1.0
...
class ReconstructorTab(UtilityTab):
    def __init__(self, id: str = "unknown", *args, **kwargs):
        super().__init__(id, *args, **kwargs)
        self.ui = Ui_reconstruction_ui()
        self.ui.setupUi(self)
        self.ui.progress_reconstruction.setValue(100)

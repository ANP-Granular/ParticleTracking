...
def compute_desired_position(
    main_window: RodTrackWindow, start: QtCore.QPoint, end: QtCore.QPoint
) -> np.ndarray:
    cam_idx = main_window.ui.camera_tabs.currentIndex()
    cam = main_window.cameras[cam_idx]
...
def post_create(
    ...
    selected_id = [
        rod.rod_id for rod in cam.rods if rod.rod_state == rn.RodState.SELECTED
    ]
    ...
    with QtCore.QReadLocker(r_data.lock):
        relevant_data = r_data.rod_data.loc[
            (r_data.rod_data.frame == frame) & (r_data.rod_data.color == color)
        ]

        saved_ids = relevant_data.particle.values
        _logger.info(f"new_id:{new_id}, saved:{saved_ids}")
        assert new_id in saved_ids
        data_pos = relevant_data.loc[
            relevant_data.particle == new_id,
            [
                f"x1_{cam.cam_id}",
                f"y1_{cam.cam_id}",
                f"x2_{cam.cam_id}",
        ...
        np.testing.assert_allclose(
            np.asarray(data_pos).squeeze(), intended_pos
        )
    return
...
def post_delete(main_window: RodTrackWindow, rod_id: int):
    ...
    raise AssertionError(
        f"Rod #{rod_id} was not found but should be at [-1, -1, -1, -1]"
    )
...
def pre_number_switch(
    main_window: RodTrackWindow, rod_id: int, new_id: int
) -> dict:
    cam_idx = main_window.ui.camera_tabs.currentIndex()
    cam = main_window.cameras[cam_idx]
    rod_ids = [rod.rod_id for rod in cam.rods]
    ...
    with QtCore.QReadLocker(r_data.lock):
        ...
        initial_state["dataset"] = data.loc[
            data.color == main_window.get_selected_color()
        ]
...
def post_number_switch(
    ...
    if cam_id == "gp3":
        ...
        raise AssertionError(
            f"Unknown camera combination used: cam1 = {cam_id}"
        )
    cam1_cols = [
        f"x1_{cam_id}",
        f"y1_{cam_id}",
    ...
    if mode is None:
        ...
        ch_r1 = changed_frame.loc[
            (changed_frame.particle == rod_id), [*cam1_cols, *cam2_cols]
        ]
        ch_r2 = changed_frame.loc[
            (changed_frame.particle == new_id), [*cam1_cols, *cam2_cols]
        ]
        in_r1 = changed_frame_init.loc[
            (changed_frame_init.particle == rod_id), [*cam1_cols, *cam2_cols]
        ]
        in_r2 = changed_frame_init.loc[
            (changed_frame_init.particle == new_id), [*cam1_cols, *cam2_cols]
        ]
        ...
        ch_r1 = changed_frame.loc[
            (changed_frame.particle == rod_id), cam1_cols
        ]
        ch_r2 = changed_frame.loc[
            (changed_frame.particle == new_id), cam1_cols
        ]
        in_r1 = changed_frame_init.loc[
            (changed_frame_init.particle == rod_id), cam1_cols
        ]
        in_r2 = changed_frame_init.loc[
            (changed_frame_init.particle == new_id), cam1_cols
        ]
...
def post_save(
    main_window: RodTrackWindow, save_path: pathlib.Path, inital_state
):
    ...
    unsaved = len(
        [
            action
            for logger in main_window.ui.lv_actions_list._loggers[-3:]
    ...
    w_data.replace(["True", "1", "1.0"], 1.0, inplace=True)
    w_data.replace(["False", "0", "0.0"], 0.0, inplace=True)
...
def post_switch_frame(
    main_window: RodTrackWindow, direction: int, inital_state: dict
):
    cam_idx = main_window.ui.camera_tabs.currentIndex()
...
def post_switch_color(
    main_window: RodTrackWindow, color: str, initial_state: dict
):

...
class UserAction(Protocol):
    def run(self, main_window: RodTrackWindow, qtbot: QtBot,
...
class CreateRod:
    ...
    def run(self, main_window: RodTrackWindow, qtbot: QtBot,
        ...
        with monkeypatch.context() as mp:
            ...
            mp.setattr(QtWidgets.QInputDialog, "getInt",
                       lambda *args, **kwargs:
...
class DeleteRod:
    ...
    def run(self, main_window: RodTrackWindow, qtbot: QtBot,
            monkeypatch: MonkeyPatch = None, tmp_path: pathlib.Path = None) \
            -> RodTrackWindow:
...
class ChangeRodPosition:
    ...
    def run(self, main_window: RodTrackWindow, qtbot: QtBot,
        ...
        warn(f"The rod #{self.rod_id} was not found."
             f"No position update has been performed.")
        return main_window


class SwitchRodNumber:
    def __init__(self, rod_id: int, new_id: int,
                 mode: NumberChangeActions = None, assertions: bool = True) \
            -> None:
        self.rod_id = rod_id
        self.new_id = new_id
        self.mode = mode
    ...
    def run(self, main_window: RodTrackWindow, qtbot: QtBot,
        ...
        for rod in rods:
            if rod.rod_id == self.rod_id:
                ...
                with monkeypatch.context() as mp:
                    mp.setattr(dialogs, "ConflictDialog",
                               lambda *args, **kwargs: my_dialog)
                    mp.setattr(my_dialog, "exec", lambda: None)
                    mp.setattr(my_dialog, "clickedButton",
                    ...
                    if self.assertions:
                        ...
                        aa.post_number_switch(main_window, self.rod_id,
                                              self.new_id, self.mode, state)
                return main_window

        warn(f"The rod #{self.rod_id} was not found."
             f"No number switch has been performed.")
        return main_window
    ...
    def run(self, main_window: RodTrackWindow, qtbot: QtBot,
        ...
        for rb in color_buttons.findChildren(QtWidgets.QRadioButton):
            if rb.objectName()[3:] == self.color:
                to_press = rb
        if to_press is None:
            warn(f"The color '{self.color}' was not found."
                 f"No color switch has been performed.")
            return main_window
...
class LengthAdjustment:
    allowed_shortcuts = [QtCore.Qt.Key_A, QtCore.Qt.Key_S,
                         QtCore.Qt.Key_R, QtCore.Qt.Key_T]

    def __init__(self, shortcut: int, rod: int = None,
                 assertions: bool = True) -> None:
...
class SelectInTree:
    ...
    def run(self, main_window: RodTrackWindow, qtbot: QtBot = None,
            monkeypatch: MonkeyPatch = None, tmp_path: pathlib.Path = None) \
            -> RodTrackWindow:
        # TODO
        return main_window
...
class OpenImage:
    def __init__(self, img_path: pathlib.Path, assertions: bool = True) \
            -> None:
    ...
    def run(self, main_window: RodTrackWindow, qtbot: QtBot,
        ...
        with monkeypatch.context() as mp:
            mp.setattr(QtWidgets.QFileDialog, "getOpenFileName",
                       lambda *args, **kwargs: (str(self.img_path), None))
            qtbot.mouseClick(main_window.ui.pb_load_images,
...
class OpenData:
    def __init__(self, data_path: pathlib.Path, assertions: bool = True) \
            -> None:
    ...
    def run(self, main_window: RodTrackWindow, qtbot: QtBot,
        ...
        with monkeypatch.context() as mp:
            ...
            # avoid blocking
            mp.setattr(dialogs, "show_warning",
                       lambda *args, **kwargs: None)
            qtbot.mouseClick(main_window.ui.pb_load_rods,
                             QtCore.Qt.MouseButton.LeftButton)
        qtbot.wait(200)
        return main_window
